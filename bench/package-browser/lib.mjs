...
function validateVariantReport(report, context, variant) {
  ...
  const webgpuVariant = variant === 'webgpu' || variant === 'firefox-webgpu';
  const firefoxVariant = variant === 'zen-webgl2' || variant === 'firefox-webgl2' || variant === 'firefox-webgpu';
  const firefoxWebGL2 = variant === 'zen-webgl2' || variant === 'firefox-webgl2';
  const envKeys = webgpuVariant
    ...
    : [...(firefoxWebGL2 ? ['execution_surface'] : []), 'ua', 'cross_origin_isolated', 'secure_context', 'webgl2', 'vendor', 'renderer', 'gpu_classification', 'performance_blocked', 'extensions', 'directional_memory', 'unavailable_metrics'];
  exactKeys(report.environment, envKeys, 'environment');
  boundedText(report.environment.ua, 'environment.ua');
  const browserUaValid = firefoxVariant
  ...
  if (webgpuVariant) {
    const requiredSurface = variant === 'firefox-webgpu' ? 'official_firefox' : 'external_chrome_extension';
    if (report.environment.execution_surface !== context.executionSurface || report.environment.execution_surface !== requiredSurface) fail(`${variant} execution surface invalid`);
    ...
    if (report.environment.webgpu !== true || !Array.isArray(report.environment.adapter_features) || !Array.isArray(report.environment.device_features)) fail(`${variant} environment invalid`);
    ...
    const requiredSurface = variant === 'zen-webgl2' ? 'zen_browser' : variant === 'firefox-webgl2' ? 'official_firefox' : null;
    if (requiredSurface && (report.environment.execution_surface !== context.executionSurface || report.environment.execution_surface !== requiredSurface)) fail(`${variant} execution surface invalid`);
  ...
  for (const [index, item] of report.cases.entries()) {
    ...
    if (webgpuVariant && (item.baseline.format === THREE_RGBA_FORMAT || item.encrypted.format === THREE_RGBA_FORMAT)) fail(`${label} WebGPU texture used RGBA fallback`);
...
export function validateFirefoxWebGL2Report(report, context) { return validateVariantReport(report, context, 'firefox-webgl2'); }
export function validateFirefoxWebGPUReport(report, context) { return validateVariantReport(report, context, 'firefox-webgpu'); }

function appendTextureResults(lines, report, readback = 'GPU readback') {
  lines.push(
...
export function renderMarkdown(report) {
  ...
  appendTextureResults(lines, report);
  return lines.join('\n');
}
...
export function renderWebGPUMarkdown(report) {
  ...
  appendTextureResults(lines, report, 'RenderTarget readback');
  return lines.join('\n');
}
...
export function renderFirefoxWebGL2Markdown(report) {
  const blocked = report.environment.performance_blocked;
  const lines = [
    '# MMDPACK Firefox WebGL2 decision', '',
    `- Status: fixed ten-case official Firefox/WebGL2 texture-entry functionality passed; performance is ${blocked ? `blocked because GPU classification is ${report.environment.gpu_classification}` : 'directional on the observed hardware GPU'}.`,
    '- Scope: plaintext or AES-256-GCM encrypted Candidate B KTX2 entry through Web Crypto, the pinned official Three.js KTX2Loader, textured-quad draw, and `gl.finish()`. This is not full-MMD first render or compositor presentation evidence.',
    `- Browser: ${report.environment.ua}`,
    `- Execution surface: ${report.environment.execution_surface} (server-authorized official Firefox).`,
    `- GPU: ${report.environment.vendor}; ${report.environment.renderer}; classification: ${report.environment.gpu_classification}.`,
    `- Three.js revision label: ${report.provenance.revision}; control SHA-256: ${report.provenance.control_sha256}.`,
    '- Key policy: Web Crypto `extractable: false`, decrypt-only. Raw key bytes were zeroed after import and were not serialized.',
    '- Timing: one warmup and five measured repetitions per lane, with baseline/encrypted order alternated for every repetition. Paired overhead is encrypted minus plaintext within the same repetition; all p50/p95 values are directional.',
...
export function renderFirefoxWebGPUMarkdown(report) {
  const blocked = report.environment.performance_blocked;
  const lines = [
    '# MMDPACK Firefox WebGPU decision', '',
    `- Status: fixed ten-case official Firefox/WebGPU texture-entry functionality passed; performance is ${blocked ? `blocked because GPU classification is ${report.environment.gpu_classification}` : 'directional on the observed hardware GPU'}.`,
    '- Scope: plaintext or AES-256-GCM encrypted Candidate B KTX2 entry through Web Crypto, the pinned official Three.js KTX2Loader, WebGPURenderer using the requested device, textured-quad draw into a 16x16 RenderTarget, `queue.onSubmittedWorkDone()`, and asynchronous RenderTarget readback. This is not full-MMD first render or compositor presentation evidence.',
    `- Browser: ${report.environment.ua}`,
    `- Execution surface: ${report.environment.execution_surface} (server-authorized official Firefox).`,
    `- Adapter: ${report.environment.adapter_info.vendor}; ${report.environment.adapter_info.architecture}; ${report.environment.adapter_info.device}; classification: ${report.environment.gpu_classification}.`,
...
export async function publishReport(report, { outputRoot, documentPath, variant = 'webgl2' }) {
  ...
  try {
    ...
    const markdown = variant === 'webgpu' ? renderWebGPUMarkdown(report)
      : variant === 'zen-webgl2' ? renderZenWebGL2Markdown(report)
        : variant === 'firefox-webgl2' ? renderFirefoxWebGL2Markdown(report)
          : variant === 'firefox-webgpu' ? renderFirefoxWebGPUMarkdown(report) : renderMarkdown(report);
