...
同じauthorityの `/firefox-webgpu` は公式Firefox WebGPU専用です。実adapter/device、GPU圧縮feature、WebGPU backendとrequested deviceの同一性を検証し、WebGL fallbackは許可しません。
...
- Firefox WebGPU判断文書: `docs/mmdpack-browser-firefox-webgpu-decision.md`（ローカルignored）
...
- Firefox WebGPU raw result: `.ai/mmdpack/browser/firefox-webgpu/runs/<browser-run-id>/report.json`（ローカルignored）
