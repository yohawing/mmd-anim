...
function validateVariantReport(report, context, variant) {
  ...
  const browserUaValid = firefoxWebGL2
...
function appendWebGL2Results(lines, report) {
  lines.push(
...
export function renderMarkdown(report) {
  ...
  appendWebGL2Results(lines, report);
  return lines.join('\n');
}
  ...
  try {
    ...
    const markdown = variant === 'webgpu' ? renderWebGPUMarkdown(report)
      : variant === 'zen-webgl2' ? renderZenWebGL2Markdown(report)
        : variant === 'firefox-webgl2' ? renderFirefoxWebGL2Markdown(report) : renderMarkdown(report);
