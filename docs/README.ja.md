...
- `mmd-anim-cli` / `mmd-anim-ffi` の `physics-bullet-native` feature で有効化できる、Bullet Physics を使った optional な MMD 物理演算 backend。必要な Bullet3 モジュールは同梱ソースから利用側の target 向けにコンパイルされるため、別途 Bullet をインストールする必要はありません。
